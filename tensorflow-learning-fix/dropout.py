...
W_L1 = tf.Variable(tf.random.truncated_normal([784, 1000], stddev=0.1))
...
# middle layer 20
W_L2 = tf.Variable(tf.random.truncated_normal([1000, 1000], stddev=0.1))
...
# neural network middle layer 2
# middle layer 20
W_L3 = tf.Variable(tf.random.truncated_normal([1000, 1000], stddev=0.1))
b_L3 = tf.Variable(tf.zeros([1000])+0.1)
W_plus_b_L3 = tf.matmul(L2, W_L3) + b_L3
L3 = tf.nn.tanh(W_plus_b_L3)
L3_drop = tf.nn.dropout(L3, 1 - (keep_prob))

# # neural network output layer
W_L4 = tf.Variable(tf.random.truncated_normal([1000, 10], stddev=0.1))
